...
- **Fase 3 (Conversa) COMEÇOU — v1.16 COMMITADA, deploy PENDENTE de autorização** — o webhook responde quando o bot é mencionado (@) ou quando alguém responde/cita mensagem dele; SÓ grupo de TESTE; anti-cascata (6/hora + cooldown 30s + anti-eco). Tabela `bot_mensagens_enviadas` criada em produção (registra o message_id de todo envio do bot). Sem busca na web/ficha relacional ainda. Ver §13.
...
- **Governança de volume (v1.12+, `agenda`/`cobranca_dia`/`ultima_chamada`):** teto
  ...
  desses três modos. **NÃO se aplica** a cobrança manual/fim_de_jogo/enviar_texto
  (gatilho manual do admin — já naturalmente espaçado). Helpers: `botLogHoje`,
...
-7. **Fase 3 (Conversa) — v1.16 COMMITADA, DEPLOY PENDENTE de autorização
   ...
   grupo de teste → resposta; responder/citar uma mensagem dele → resposta;
   mensagem comum → só arquiva.
