...
> - **HOME = ABA RANKING (20/07, decisão final do Vini):** a home abre direto na
>   aba **Ranking** (`activeTab='ranking'` + classes `.active` estáticas do nav
>   e das divs de aba trocadas JUNTAS), que já abre no recorte "Mata-Mata" (o
>   ranking oficial: pódio Du/ChatGPT Leo/Ratazana00). A 1ª tentativa (seção
>   `mmFinalRankingHTML` DENTRO da aba Mata-mata, merge `08908de`/safepoint
>   `v27-prod-pre-home-ranking`) foi REVERTIDA a pedido do Vini (revert
>   `5fd3236`) — não recriar. Histórico dos merges cirúrgicos (só `index.html`,
>   sempre com autorização): `46b72ff` filtro/admin na FINAL (`v26-prod-pre-
>   home-final`) · `ae041ed` aba Ranking como home + revert (`v28-prod-pre-
>   home-aba-ranking`) — confirmado ao vivo em produção. ⚠️ O banco DEV
>   (`dev_mata_*`) segue DESATUALIZADO (quartas+ abertas lá) — números do
>   preview DEV ficam tortos até alguém rodar o Espelhar prod→dev (o
>   classificador bloqueia a automação de rodar o espelho).
...
Palpite de IA (grupos): campo `g/c/d/e` no GT. **Palpite de IA (mata): editável DIRETO no app, igual aos humanos, SEM toggle.** No `mmCardBody`: humanos usam o layout GRANDE (`mm-hcell` + `mmHumPredHTML`, foto 50px); **IAs usam o layout COMPACTO de sempre (`mm-iacell`, foto 30px ao lado do nome) com campos EDITÁVEIS** — o antigo box cinza de leitura (`mm-iascore`) virou dois inputs pequenos (`mmIaPredHTML` + CSS `.mm-iapred`), gravando pelo MESMO caminho (`mmUpdPalpite`→`mmUpsertPalpite`→`mata_palpites`), mesma trava de kickoff (disabled/cinza em jogo começado), "quem passa" clicável. `mmUpdPalpite` acha o input por `[data-pid]` (serve humano E IA compacta). Qualquer um edita na tela de palpites do app (modelo sem-auth de sempre). ⚠️ **Histórico:** v1 era gated (`localStorage['ratz_edit_ia']`, removida — falhava na prática); v2 (v21) rendia IA com o layout grande do humano e os cards ficaram enormes; **v3 (v22) trouxe de volta o card compacto mantendo a edição** (estado ATUAL). Se algum texto citar `mmIaEdit`/`ratz_edit_ia`, está desatualizado. Só front-end (o bot já lê `mata_palpites`). Testado com save+reload+persistência.
