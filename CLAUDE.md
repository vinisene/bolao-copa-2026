...
**Zebras das quartas** (`MM_ZEBRA`, odds Estrelabet 07/07 — todas ZEBRA +3, nenhum zebrão): `qf_1` lado A (Marrocos, odd 6.70 ~14,7%) · `qf_2` lado B (Bélgica, odd 5.98 ~16,5%) · `qf_3` lado A (Noruega, odd 4.28 ~23,0%) · `qf_4` lado B (Suíça, odd 5.46 ~18,1%). Replicadas nos 3 lugares (index.html, admin, Edge Function) na leva de 07/07 — ⚠️ **função ainda NÃO redeployada e `ratazana` ainda NÃO mesclada pra `main` com isso** (ver §15).
