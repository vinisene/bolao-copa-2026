...
- **Função v1.12 DEPLOYADA (versão 15, jul/2026)** — dois modos novos pra automação (Fase 4): `?tipo=agenda` (painel do dia + alerta de liderança) e `?tipo=ultima_chamada` (T-30min por jogo), com teto diário de mensagens e idempotência — ver §13. **pg_cron/pg_net AINDA NÃO habilitados no banco** (auditoria confirmou zero automação hoje): SQL pronto em `supabase_pg_cron.sql`, aguardando o Vini rodar no SQL Editor (armadilha 8 — DDL de extensão é automação bloqueada). Até lá, os dois modos novos só disparam se alguém chamar a URL manualmente.
...
- **Automação (pg_cron):** `supabase_pg_cron.sql` — habilita `pg_cron`/`pg_net` e agenda os jobs de agenda/última chamada. Ainda não rodado — ver §13/§15.
...
- **Modos** (`?token=<BOT_TRIGGER_TOKEN>` obrigatório em todos):
  ...
  - **`&tipo=agenda&destino=...[&force=1]`** (v1.12) → **AGENDA DO DIA**, pensada pra
    pg_cron 1x/dia às 9h: jogos de hoje (turbo/zebra/quem falta palpitar) + o ÚNICO
    alerta de liderança detalhado do dia (a persona já previa isso: "alerta de
    liderança detalhado só na futura mensagem de agenda das 9h"). Sem jogo hoje
    (ou todos já finalizados) → não envia nada, só loga (modo entressafra fica pra
    outra sessão). Idempotente (1x por dia+destino; `&force=1` ignora só essa trava,
    nunca o teto).
  - **`&tipo=ultima_chamada&destino=...[&force=1]`** (v1.12) → **ÚLTIMA CHAMADA**,
    pensada pra pg_cron a cada ~10min: só age nos jogos que estão entrando na janela
    de 15–30min antes do kickoff (cutuca quem falta palpitar daquele jogo específico).
    Idempotente por jogo (não repete o mesmo jogo no mesmo dia).
- **Governança de volume (v1.12, só para `agenda`/`ultima_chamada`):** teto diário de
  mensagens automáticas — **4 mensagens em dia com jogo, 2 em dia sem jogo**
  (`TETO_MSGS_DIA_COM_JOGO`/`TETO_MSGS_DIA_SEM_JOGO` no código) — conta QUALQUER
  mensagem enviada com sucesso naquele dia+destino (`bot_log`), não só as desses dois
  modos. **NÃO se aplica** a cobrança/fim_de_jogo/enviar_texto (gatilho manual do
  admin ou evento real de jogo — já naturalmente espaçados). Helpers: `botLogHoje`,
  `contaEnviadasHoje`, `jaRodouHoje`, `limitesBrasiliaHojeISO`.
- **pg_cron:** o SQL pra habilitar `pg_cron`/`pg_net` e criar os dois jobs está em
  `supabase_pg_cron.sql` (destino `teste` nos dois, de propósito — trocar pra `oficial`
  é decisão explícita do Vini, editando o `command` do job). **Ainda NÃO rodado** —
  automação de DDL/extensão é bloqueada pro Claude Code (armadilha 8); preparado pro
  Vini rodar no SQL Editor.
...
-2. **✅ v1.12 DEPLOYADA (versão 15, jul/2026)** — modos `agenda` e `ultima_chamada`
   no ar (código + deploy confirmados). **AÇÃO RESTANTE DO VINI: rodar
   `supabase_pg_cron.sql` no SQL Editor** pra habilitar `pg_cron`/`pg_net` e criar
   os 2 jobs (agenda 9h, última chamada a cada 10min) — sem isso os modos existem
   mas não disparam sozinhos. Auditoria confirmou zero automação hoje (pg_cron nem
   instalado). Destino inicial dos jobs = `teste` (trocar pra `oficial` é decisão
   separada, editando o `command` do job). Teste funcional em runtime (chamar a URL
   de verdade) não foi feito nesta sessão — exige o `BOT_TRIGGER_TOKEN` real, que
   por design só existe no Supabase e no navegador de quem administra, não neste
   ambiente. Fora do escopo desta leva (ficam pra quando o Vini priorizar):
   apito inicial, resumo pós-rodada com troféus/Humanos×Máquinas, modo entressafra.
...
2. **Fases 2–3 do bot** — webhook (responder mensagens do grupo), conversa. Ainda não
   começadas; virão em prompts futuros. **Fase 4 (agendamentos automáticos) começou**:
   agenda 9h + última chamada T-30min no ar (v1.12, ver §13/item -2); faltam pg_cron
   habilitado, apito inicial, resumo pós-rodada e modo entressafra.
