...
**Fase ativa: OITAVAS** (16avos 100% encerrado, 16/16 jogos; oitavas em 4/8 — r16_1 a r16_4 fechados). `bot_config.fase_ativa = 'oitavas'`. **Desde a v1.19, isso avança SOZINHO** quando o último jogo da fase é fechado no admin (se a próxima fase já tiver os confrontos cadastrados) — o Table Editor manual vira só via de emergência pra quando o sistema não conseguir decidir sozinho (ver §13/§15 item -13).
