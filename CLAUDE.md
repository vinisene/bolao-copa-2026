...
2. **Fases do bot** — **Fase 2 (Ouvidos: captura de mensagens) COMEÇOU** na v1.14
   (ver item -4). **Fase 3 (responder menção/citação/conversa) NÃO começou** — vem
   em prompt futuro, sobre os dados capturados pela Fase 2. **Fase 4 (agendamentos
   automáticos) começou**: agenda 9h + cobrança 9h01 + última chamada T-60min no ar
   (v1.13, ver §13); faltam pg_cron habilitado, apito inicial, resumo pós-rodada e
   modo entressafra.
