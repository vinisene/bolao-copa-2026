...
Palpite de IA (grupos): campo `g/c/d/e` no GT. **Palpite de IA (mata): editável DIRETO no app (v2/v21), igual aos humanos, SEM toggle.** `mmCardBody` renderiza TODAS as células (humanos e IAs) com `mmHumPredHTML` — mesmo input lime/editável, mesmo `mmUpdPalpite`→`mmUpsertPalpite`→`mata_palpites`, mesma trava de kickoff (input disabled/cinza só em jogo já começado). A classe `human`/`ia` só define fundo/coluna (IA continua na coluna das IAs). Qualquer um edita na tela de palpites do app (é o modelo sem-auth de sempre). ⚠️ **A v1 gated (`localStorage['ratz_edit_ia']`, botão no admin, `mmIaEdit`, `?editaias=`) foi REMOVIDA** — falhava na prática (sem ligar a flag, IA aparecia como caixa cinza de leitura); se algum texto antigo citar `mmIaEdit`/`ratz_edit_ia`, está desatualizado. Só front-end (o bot já lê `mata_palpites`). Testado com save+reload+persistência (ver §15).
